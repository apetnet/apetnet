...
import ru.apetnet.ext.independent.kotlin.minus
import ru.apetnet.pnutil.extension.findIndicesMoreThanZero
...
/**
 * Experimental (with omega-support)
 */
internal class VertexHelperImpl2 : VertexHelper {
     ...
     * Find
     */
    private fun findOmega(
        marking: DoubleArray,
        tId: Int
    ): List<Int> {
        val omegaList = mutableListOf<Int>()

        for (i in 0 until idList.size) {
            val wId = idList[i]
            val wTId = tMap[wId]
            val wMarking = getMarking(wId)

            val markingDiff = marking - wMarking

            if (tId == wTId && markingDiff.all { it >= 0.0 }) {
                omegaList.addAll(markingDiff.findIndicesMoreThanZero())
                break
            }
        }
    ...
    private fun buildMarking(
        ...
        val omegaList = findOmega(marking, tId)
        val isOmega = omegaList.isNotEmpty()
